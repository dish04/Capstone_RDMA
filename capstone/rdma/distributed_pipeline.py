...
def run_node(args):
    ...
    if rank == 0 and args.json_output:
        ...
        mean_step = _calc_mean(step_lats)
        summary = {
            "backend": backend,
            "total_nodes": total_nodes,
            "num_steps": len(step_metrics),
            "step_metrics": step_metrics,
            "mean_step_latency_ms": round(mean_step, 2),
            "mean_compute_latency_ms": round(_calc_mean(comp_lats), 2),
            "mean_network_latency_ms": round(_calc_mean(net_lats), 2),
            "tokens_per_second": round(1000.0 / mean_step, 2) if mean_step > 0 else 0.0
        }
        with open(args.json_output, "w") as f:
            json.dump(summary, f, indent=2)
        print(f"\n[Metrics] Saved benchmark report to {args.json_output}")
        print(f"[Metrics] Mean Step Latency : {summary['mean_step_latency_ms']:.2f} ms")
        ...
        print(f"[Metrics] Generation Speed  : {summary['tokens_per_second']:.2f} tokens/sec")
